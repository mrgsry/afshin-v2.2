...
function service_report_extract_json_object($text) {
    ...
    if (is_array($result) && (array_key_exists('customer_id', $result) || array_key_exists('date_doc', $result) || array_key_exists('activities', $result))) return $result;
    if (is_array($result)) {
        foreach (['data', 'output', 'result', 'service_report'] as $key) {
...
function service_report_text_from_content($content) {
    if (is_string($content)) return $content;
    if (!is_array($content)) return '';
    if (array_keys($content) !== range(0, count($content) - 1)) {
        foreach (['text', 'content', 'value'] as $key) {
            if (array_key_exists($key, $content)) return service_report_text_from_content($content[$key]);
        }
    ...
    foreach ($content as $part) {
        if (is_string($part)) {
            ...
            $text .= (string)($part['text'] ?? $part['content'] ?? $part['value'] ?? '');
        }
    }
    return $text;
}

function service_report_openai_response_text($response, $data) {
    $text = '';
    if (is_array($data)) {
        $choice = $data['choices'][0] ?? [];
        $message = is_array($choice['message'] ?? null) ? $choice['message'] : [];
        $text = service_report_text_from_content($message['content'] ?? '');
        if ($text === '') $text = service_report_text_from_content($choice['text'] ?? '');
        if ($text === '') $text = service_report_text_from_content($message['reasoning_content'] ?? '');
        if ($text === '') $text = service_report_text_from_content($data['output_text'] ?? $data['output'] ?? '');
    }
    if ($text !== '') return $text;

    // Some compatible endpoints always return SSE, even when streaming was not requested.
    $chunks = [];
    foreach (preg_split('/\R/', (string)$response) as $line) {
        ...
        $piece = service_report_text_from_content($delta['content'] ?? '');
        if ($piece === '') $piece = service_report_text_from_content($chunkData['choices'][0]['text'] ?? '');
...
if ($config['provider'] === 'openai_compatible') {
    $payload = json_encode(['model' => $config['model'], 'messages' => [['role' => 'system', 'content' => 'Balas hanya JSON valid tanpa markdown. Jangan gunakan code fence atau penjelasan tambahan.'], ['role' => 'user', 'content' => $prompt]], 'temperature' => 0.1, 'max_tokens' => 4096]);
...
$text = $config['provider'] === 'openai_compatible'
    ? service_report_openai_response_text($response, $data)
    : service_report_text_from_content($data['candidates'][0]['content']['parts'] ?? '');
